...
import { MapProdiMinatService } from 'src/app/_services/map-prodi-minat.service';
import { Observable } from 'rxjs';
import { AdminManagementService } from 'src/app/_services/admin-management.service';

@Component({
  selector: 'app-adm-map-prodi-minat',
  templateUrl: './adm-map-prodi-minat.component.html',
  styleUrls: ['./adm-map-prodi-minat.component.scss']
})
export class AdmMapProdiMinatComponent implements OnInit {
  ...
  constructor(
    ...
    private mapProdiMinatService: MapProdiMinatService
  ) {
  ...
  ngOnInit() {
    this.loadProfile();
    this.loadDokument();
    this.mapProdiMinatService.getListProdi().subscribe(item => {
       this.listSelectionProdi = item.data.map((e)=>{
        return { value : e.id, label : e.nama_prodi}
       })
    })
    this.mapProdiMinatService.getListMap().subscribe(item => {
  ...
  loadDokument(){
    this.mapProdiMinatService.getListMinat().subscribe(item => {
       this.documents = item.datas;
    })
  }
